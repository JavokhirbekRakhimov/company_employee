...
begin
    in_name=(select name from
        json_populate_record(Null::role_type,in_json::json) limit 1);
    ...
    if success then
        ...
        insert into role(name, rank) select name,rank from json_populate_record(Null::role_type,in_json::json) returning id into in_id;
